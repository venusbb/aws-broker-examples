...
def check_secrets():
    ...
    environ['SQS_QUEUE_ARN'] = vcap_services['sqs'][0]['credentials']['QUEUE_ARN']
    environ['SQS_QUEUE_NAME'] = vcap_services['sqs'][0]['credentials']['QUEUE_NAME']
    environ['SQS_QUEUE_URL'] = vcap_services['sqs'][0]['credentials']['QUEUE_URL']
    environ['SQS_AWS_ACCESS_KEY_ID'] = vcap_services['sqs'][0]['credentials']['SQS_AWS_ACCESS_KEY_ID']
    environ['SQS_AWS_SECRET_ACCESS_KEY'] = vcap_services['sqs'][0]['credentials']['SQS_AWS_SECRET_ACCESS_KEY']
...
def put_item(item_id, **kwargs):
    ...
    kwargs['client'].send_message(
        ...
        # MessageGroupId='testgroup'  # Doesn't exist for standard queues
    )
    return
